...
app_ui.py         일정을 추가, 삭제, 저장하고 분석할 수 있는 Streamlit UI입니다.
...
- 데일리 브리핑 생성
  ...
  - 리스크 메시지
  - 추천 액션
- Streamlit UI에서 날짜 선택, 일정 추가, 일정 삭제, 일정 저장, 분석 실행
...
UI는 한국어로 표시됩니다. `캘린더` 탭에서 날짜를 선택하고 일정을 직접 추가하거나 삭제할 수 있으며, `현재 일정 저장하기` 버튼으로 `data/sample_schedule.json`에 반영할 수 있습니다. `데일리 브리핑` 탭에서는 선택한 날짜의 일정만 분석해 리스크와 추천 액션을 확인할 수 있습니다.
