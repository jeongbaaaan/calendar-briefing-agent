...
| GET | `/briefing` | 일정 분석 결과, 일정 유형, 브리핑 결과, 응원 메시지 반환 |
...
- `encouragement`: 오늘의 작은 응원과 부담을 낮추는 리마인더

## UI Preview

React UI는 밝은 배경, 넓은 여백, 부드러운 카드, 전체 캘린더, 세로 타임라인을 사용합니다.

- 상단에는 `오늘 일정 브리핑` 히어로 영역과 날짜/타임존 표시가 있습니다.
- 왼쪽에는 월간 캘린더가 표시되고, 오늘 브리핑이 가능한 날짜가 강조됩니다.
- 선택한 날짜 아래에는 해당 날짜의 일정이 시간순 타임라인 카드로 표시됩니다.
- 오른쪽에는 선택한 날짜 기준의 `일정 수`, `예정된 시간`, `주의 구간`, `일정 유형` 요약 카드가 있습니다.
- `오늘 일정 분석하기` 버튼을 누르면 `오늘 하루 요약`, `조심할 점`, `미리 챙기면 좋은 것`, `오늘의 작은 응원`, `나의 일정 유형` 카드가 나타납니다.
